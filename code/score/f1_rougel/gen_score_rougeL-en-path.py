...
def calculate_sentence_scores(test_data, generated_data, threshold, start_id=None, end_id=None):
    ...
    for id_, test_data in tqdm(test_data.items(),desc = 'Processing'):
        ...
        if test_text == "" or generated_text == "" or type(test_text) != str or type(generated_text) != str:
            similarity = 0
        else:
            rouge = Rouge()
            scores = rouge.get_scores(test_text, generated_text)
            similarity = scores[0]['rouge-l']['f']
        logging.info(f"Value: {test_text} | Similarity Scores: {similarity}")
        score_list.append((test_text, similarity, "score-test"))
        sentence_scores.setdefault(id_, []).append(([test_text, generated_text], similarity))
...
# 新的路径部分
log_str = "log/rougel_scores-en-gen.log"
...
with open(test_jsonl_file, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if id_ and output and (task.endswith("-en-gen")):
            test_data[id_] = {
                "text": output,
                "task": task
            }
...
start_id = 901  # 开始ID范围（可选）
end_id = 1200  # 结束ID范围（可选）
